...
class SeekingController extends Controller
{
    ...
    public function get_users_my_type(request $request)
    {
     $user = Auth::user(); 
      ...
      ->select("users.id","users.profileName",'users.maritalBeliefSystem','users.spiritualValue',"users_location.city","latitude","longitude","state","state_abbr",DB::raw("6371 * acos(cos(radians(" . $Location->latitude . ")) 
                        * cos(radians(latitude)) 
                        * cos(radians(longitude) - radians(" .  $Location->longitude . ")) 
                        + sin(radians(" . $Location->latitude. ")) 
                        * sin(radians(latitude))) AS distance"
